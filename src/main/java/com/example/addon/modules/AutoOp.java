...
public class AutoOp extends Module {
    ...
    private final Setting<List<String>> extraCommands = sgCommands.add(new StringListSetting.Builder()
        .name("extra-commands")
        .description("Extra commands to run right after /op. Use {user} for your name, {target} for bot name.")
        .defaultValue(new ArrayList<>())
        .build()
    );

    private final Setting<Integer> extraCommandsDelay = sgCommands.add(new IntSetting.Builder()
        .name("extra-commands-delay")
        .description("Ticks to wait before sending extra commands after /op.")
        .defaultValue(20)
        .min(0)
        .sliderMax(200)
        .build()
    ...
    private int extraCommandTicker = 0;
    private volatile boolean extraCommandsPending = false;
    ...
    public AutoOp() {
        super(AddonTemplate.CATEGORY, "auto-op", "Probes players with /op to find a privileged account, then ops you.");
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        switch (state) {
            ...
            case PROBING -> {
                if (ticker >= 40) {
                    info("Sending /op " + myName);
                    sendCommand("op " + myName);
                    extraCommandsPending = true;
                    extraCommandTicker = 0;
                    state = State.WAITING_PROBE;
                    ticker = 0;
                }
            }
            case WAITING_PROBE -> {
                if (extraCommandsPending) {
                    extraCommandTicker++;
                    if (extraCommandTicker >= extraCommandsDelay.get()) {
                        extraCommandsPending = false;
                        sendExtraCommands();
                    }
                }
                if (probeSucceeded) {
            ...
            case RUNNING_CLEANUP -> {
                if (ticker >= 40) {
                    String cmd = cleanupCommand.get()
                        .replace("{user}", myName)
                        .replace("{target}", currentBot);
                    sendCommand(cmd);
    ...
    private void sendExtraCommands() {
        for (String cmd : extraCommands.get()) {
            String resolved = cmd
                .replace("{user}", myName)
                .replace("{target}", currentBot);
    ...
    private void spawnSession() {
        ...
        session.addListener(new SessionAdapter() {
            ...
            public void disconnected(DisconnectedEvent event) {
                ...
                if (!reason.equals("done") && !reason.equals("switching") && !reason.equals("no permission")) {
    ...
    private void disconnectSession(String reason) {
        if (session != null) {
            try { if (session.isConnected()) session.disconnect(reason); } catch (Exception ignored) {}
            session = null;
        }
    }
