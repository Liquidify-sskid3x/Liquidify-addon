...
public class MusicPlayerModule extends Module {
    ...
    public final Setting<Integer> scrollSpeed=sg.add(new IntSetting.Builder().name("title-scroll-speed").description("Ticks between each scroll step. Lower = faster.").defaultValue(2).min(1).max(50).sliderMin(1).sliderMax(50).build());
    ...
    private volatile long dlTotal=0;
    ...
    public String getScrollingTitle(){
        String t=AudioPlayer.get().getTitle();
        if(t==null||t.length()<=MAX_CHARS)return t==null?"":t;
        String p=t+"     ";int len=p.length();int s=scrollOff%len;
        return(p+p).substring(s,s+MAX_CHARS);}
    public void tickScroll(){
        String t=AudioPlayer.get().getTitle();
        if(t==null||t.length()<=MAX_CHARS){scrollOff=0;scrollTick=0;return;}
        scrollTick++;
        if(scrollOff==0&&scrollTick<PAUSE_TICKS)return;
        String p=t+"     ";
        if(scrollOff>=p.length()&&scrollTick%PAUSE_TICKS!=0)return;
        if(scrollTick%scrollSpeed.get()==0){scrollOff++;if(scrollOff>=p.length()){scrollOff=0;scrollTick=0;}}}
    ...
    private void addUrl(String url){
        ...
        loading=true;dlProgress=-1;dlBytes=0;dlTotal=0;
        status="Starting download...";reload();
        e.extractWithProgress(url,(bytes,total)->{
            ...
            status="Error: "+trunc(m,50);reload();return null;});}
    ...
    private void build(GuiTheme theme,WVerticalList root){
        ...
        if(!e.isReady()){
            ...
            if(e.isDownloadingFfmpeg()){
                double pct=e.getFfmpegProgress();
                String pctStr=pct>=0?String.format(" %.1f%%",pct):"";
                root.add(theme.label("Downloading ffmpeg..."+pctStr+" "+formatBytes(e.getFfmpegDownloadedBytes()))).widget();}
        ...
        if(loading){
            root.add(theme.label(status)).widget();
            if(dlProgress>=0){
                root.add(theme.label(buildProgressBar(dlProgress)+" "+String.format("%.1f%%",dlProgress))).widget();
                if(dlTotal>0)root.add(theme.label(formatBytes(dlBytes)+" / "+formatBytes(dlTotal))).widget();}}
    ...
    private String buildProgressBar(double pct){
        int total=20;int filled=(int)(pct/100.0*total);
